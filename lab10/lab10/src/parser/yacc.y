...
%token COUNT MAX MIN SUM AS LIMIT
// non-keywords
%token LEQ NEQ GEQ T_EOF BIGINT DATETIME
...
%type <sv_col> col
%type <sv_cols> colList selector aggSelector aggList aggExpr
...
type:
    ...
    |   DATETIME
    {
        $$ = std::make_shared<TypeLen>(SV_TYPE_DATETIME, sizeof(long long));
    }
    ;
...
aggExpr:
        COUNT '(' '*' ')' AS colName
    {
        $$ = std::vector<std::shared_ptr<Col>>{
            std::make_shared<Col>(AGG_COUNT, std::make_shared<Col>("", "*"), $6)};
    }
    |   COUNT '(' ')' AS colName
    {
        $$ = std::vector<std::shared_ptr<Col>>{
            std::make_shared<Col>(AGG_COUNT, std::make_shared<Col>("", "*"), $5)};
    }
    |   COUNT '(' col ')' AS colName
    {
        $$ = std::vector<std::shared_ptr<Col>>{std::make_shared<Col>(AGG_COUNT, $3, $6)};
    }
    |   MAX '(' col ')' AS colName
    {
        $$ = std::vector<std::shared_ptr<Col>>{std::make_shared<Col>(AGG_MAX, $3, $6)};
    }
    |   MIN '(' col ')' AS colName
    {
        $$ = std::vector<std::shared_ptr<Col>>{std::make_shared<Col>(AGG_MIN, $3, $6)};
    }
    |   SUM '(' col ')' AS colName
    {
        $$ = std::vector<std::shared_ptr<Col>>{std::make_shared<Col>(AGG_SUM, $3, $6)};
    }
    ;
