...
std::shared_ptr<Query> Analyze::do_analyze(std::shared_ptr<ast::TreeNode> parse)
{
    ...
    if (auto x = std::dynamic_pointer_cast<ast::SelectStmt>(parse))
    {
        ...
        for (auto &sv_set_clause : x->set_clauses) {
            ...
            // Check type compatibility (allow implicit conversion among INT/BIGINT/FLOAT)
            auto is_int_family = [](ColType t) {
                return t == TYPE_INT || t == TYPE_BIGINT || t == TYPE_FLOAT;
            };
            bool set_compatible = (col->type == set_clause.rhs.type) ||
                                  (is_int_family(col->type) && is_int_family(set_clause.rhs.type));
            if (!set_compatible) {
                throw IncompatibleTypeError(coltype2str(col->type), coltype2str(set_clause.rhs.type));
            }
            // Implicit type conversion to match the column type
...
void Analyze::check_clause(const std::vector<std::string> &tab_names, std::vector<Condition> &conds) {
    ...
    for (auto &cond : conds) {
        ...
        if (cond.is_rhs_val) {
            ...
            // Implicit type conversion among INT/BIGINT/FLOAT
        ...
        // Allow comparison among INT/BIGINT/FLOAT (implicit conversion)
        auto is_cmp_int_family = [](ColType t) {
            return t == TYPE_INT || t == TYPE_BIGINT || t == TYPE_FLOAT;
        };
        bool compatible = (lhs_type == rhs_type) ||
                          (is_cmp_int_family(lhs_type) && is_cmp_int_family(rhs_type));
        if (!compatible) {
...
Value Analyze::convert_sv_value(const std::shared_ptr<ast::Value> &sv_val) {
    ...
    if (auto int_lit = std::dynamic_pointer_cast<ast::IntLit>(sv_val)) {
        // Check for overflow: compare raw_str against BIGINT bounds.
        // strtoll clamps on overflow, so we must check the original string.
        if (!int_lit->raw_str.empty()) {
            std::string s = int_lit->raw_str;
            bool negative = false;
            size_t start = 0;
            if (s.size() > 0 && s[0] == '+') { start = 1; }
            else if (s.size() > 0 && s[0] == '-') { negative = true; start = 1; }
            while (start < s.size() - 1 && s[start] == '0') start++;
            std::string digits = s.substr(start);
            std::string max_str = "9223372036854775807";
            std::string min_str = "9223372036854775808";
            auto cmp_abs = [](const std::string &a, const std::string &b) -> int {
                if (a.size() != b.size()) return a.size() < b.size() ? -1 : 1;
                return a.compare(b);
            };
            if (negative) {
                if (cmp_abs(digits, min_str) > 0)
                    throw IncompatibleTypeError("BIGINT", "overflow");
            } else {
                if (cmp_abs(digits, max_str) > 0)
                    throw IncompatibleTypeError("BIGINT", "overflow");
            }
        }
        long long v = int_lit->val;
        if (v > INT32_MAX || v < INT32_MIN) {
            // Needs BIGINT storage
            val.set_bigint(v);
        } else {
