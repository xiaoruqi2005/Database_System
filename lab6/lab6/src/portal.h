    ...
    std::shared_ptr<PortalStmt> start(std::shared_ptr<Plan> plan, Context *context)
    {
        ...
        if (auto x = std::dynamic_pointer_cast<OtherPlan>(plan)) {
            ...
            switch(x->tag) {
                case T_select:
                {
                    // Check whether this is an aggregate plan or a projection plan
                    if (auto agg_plan = std::dynamic_pointer_cast<AggregatePlan>(x->subplan_)) {
                        std::unique_ptr<AbstractExecutor> root = convert_plan_executor(agg_plan, context);
                        // Build sel_cols from aggregate aliases for output header
                        std::vector<TabCol> sel_cols;
                        for (auto &agg : agg_plan->aggs_) {
                            sel_cols.push_back({.tab_name = "", .col_name = agg.alias});
                        }
