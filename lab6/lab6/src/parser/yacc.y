...
%type <sv_agg_type> agg_func
%type <sv_agg> agg_expr
...
dml:
    ...
    |   SELECT agg_list FROM tableList optWhereClause opt_order_clause
    {
        ...
        auto stmt = std::make_shared<SelectStmt>(std::vector<std::shared_ptr<Col>>{}, $4, $5, $6);
        stmt->aggs = $2;
        $$ = stmt;
    }
    ;
...
agg_expr:
        agg_func '(' col ')' opt_as_alias
    {
        $$ = std::make_shared<AggExpr>($1, false, $3, $5);
    }
    |   agg_func '(' '*' ')' opt_as_alias
    {
        // COUNT(*) - only COUNT supports * as argument
        $$ = std::make_shared<AggExpr>($1, true, nullptr, $5);
    }
    ;
