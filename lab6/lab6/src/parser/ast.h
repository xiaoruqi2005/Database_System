...
// Aggregate function types for SUM/MAX/MIN/COUNT
enum AggType {
...
// Aggregate function expression node, e.g. SUM(col), MAX(col), COUNT(*)
struct AggExpr : public TreeNode {
    AggType agg_type;                   // function type: SUM/MAX/MIN/COUNT
    bool is_count_star;                 // true for COUNT(*), false otherwise
    std::shared_ptr<Col> col;           // argument column (nullptr for COUNT(*))
    std::string alias;                  // AS alias name (may be empty)

    AggExpr(AggType agg_type_, bool is_count_star_, std::shared_ptr<Col> col_, std::string alias_) :
            agg_type(agg_type_), is_count_star(is_count_star_), col(std::move(col_)), alias(std::move(alias_)) {}
};
...
struct SelectStmt : public TreeNode {
    ...
    std::vector<std::shared_ptr<AggExpr>> aggs;     // aggregate expressions (empty for normal select)
