...
std::shared_ptr<Query> Analyze::do_analyze(std::shared_ptr<ast::TreeNode> parse) {
    ...
    if (auto x = std::dynamic_pointer_cast<ast::SelectStmt>(parse)) {
        ...
        for (auto &sv_sel_col : x->cols) {
            if (sv_sel_col->agg_type != ast::AGG_NONE) {
                has_aggregate = true;
                if (!sv_sel_col->count_star) {
                    TabCol checked = check_column(all_cols, {.tab_name = sv_sel_col->tab_name, .col_name = sv_sel_col->col_name});
                    sv_sel_col->tab_name = checked.tab_name;
                    sv_sel_col->col_name = checked.col_name;
                    auto &tab = sm_manager_->db_.get_table(checked.tab_name);
                    auto col = tab.get_col(checked.col_name);
                    if (sv_sel_col->agg_type == ast::AGG_SUM && !is_numeric_type(col->type)) {
                        throw IncompatibleTypeError("SUM", coltype2str(col->type));
                    }
                    query->cols.push_back(checked);
                }
                query->aggregate_cols.push_back(sv_sel_col);
            } else {
                query->cols.push_back({.tab_name = sv_sel_col->tab_name, .col_name = sv_sel_col->col_name});
            }
        }
        if (has_aggregate && query->aggregate_cols.size() != x->cols.size()) {
            throw InternalError("aggregate and normal columns cannot be mixed");
        }
        if (query->cols.empty() && !has_aggregate) {
